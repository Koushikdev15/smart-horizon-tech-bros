...
const statusStyles: Record<string, string> = {
  'Completed': 'bg-emerald-100 dark:bg-emerald-950/60 border-emerald-300 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400',
...
export default function BlockchainTimeline({ events, compact = false }: BlockchainTimelineProps) {
  return (
    <div className="relative">
      {/* Vertical line */}
      <div className="absolute left-5 top-8 bottom-0 w-0.5 bg-gradient-to-b from-emerald-400/60 via-emerald-300/30 to-transparent" />
            ...
            <div key={index} className="relative flex gap-4 pl-0">
              ...
              <div
                className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-all
                  ${isCompleted ? 'bg-emerald-500 border-emerald-500 text-white shadow-md shadow-emerald-200 dark:shadow-emerald-900' :
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium
                          ${isCompleted ? 'bg-emerald-200 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-300' :
