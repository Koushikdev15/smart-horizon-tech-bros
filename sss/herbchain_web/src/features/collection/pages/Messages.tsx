...
const roleColors: Record<string, string> = {
  ...
  'Collection Center': 'bg-primary/10 text-primary dark:bg-primary/16 dark:text-primary',
          ...
          <CardContent className="flex-1 overflow-y-auto p-2 space-y-1.5">
            {threads.map((thread) => (
              <button
                ...
                className={`w-full text-left p-3 rounded-lg border transition-colors ${selected.batchId === thread.batchId ? 'border-primary/35 bg-primary/6 dark:bg-primary/12' : 'border-border hover:bg-muted/50'}`}
              >
                <div className="flex items-center justify-between gap-2 mb-1">
                  ...
                  {thread.messages.some(m => !m.isRead) && <div className="w-2 h-2 bg-primary rounded-full shrink-0" />}
                </div>
                ...
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    <div className="flex items-center gap-1.5">
                      {!isMe && (
                        <div className="w-6 h-6 rounded-full bg-primary/10 dark:bg-primary/16 flex items-center justify-center text-[10px] font-bold text-primary">{msg.senderName.charAt(0)}</div>
                      )}
                    ...
                    <div className={`px-4 py-2.5 rounded-2xl text-sm ${isMe ? 'bg-primary text-white rounded-tr-sm' : 'bg-muted rounded-tl-sm'}`}>
                      {msg.content}
                    </div>
            ...
            {selected.isReadOnly ? (
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="flex-1"
                />
                <Button onClick={sendMessage} disabled={!newMsg.trim()} className="bg-primary hover:bg-primary text-white px-3">
