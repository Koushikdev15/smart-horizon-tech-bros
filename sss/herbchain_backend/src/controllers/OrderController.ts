...
export class OrderController {
  ...
  placeOrder = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.orderService.placeOrder(req.user.id, value);
  ...
  getOwn = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const result = await this.orderService.getOwn(req.user.id);
  ...
  getOwnById = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const result = await this.orderService.getOwnById(req.user.id, req.params.id as string);
